...
parser.add_argument('--infile',type = str, default = 'car_csv/test_car.csv')
...
parser.add_argument('--point_scales_list',type=list,default=[818,512],help='number of points in each scales') # set the first number of '--point_scales_list' to (point_number + 512).
...
car_ids = [filename.split('/')[1] for filename in glob.glob('test_files/*.npy')]
print(car_ids)
for i, car_id in enumerate(car_ids):
	test_npy = np.load(os.path.join('test_files/', car_id), allow_pickle = True)

	"""
	# use following code when have bounding box data
	...
	test_npy = np.dot(test_npy - center, rotation) / scale
	test_npy = np.dot(test_npy, [[1, 0, 0], [0, 0, 1], [0, 1, 0]])
	numpy.savetxt(os.path.join('car_csv/', car_id.split('.')[0]+'.csv'), test_npy, delimiter=",")
	"""
...
np.savetxt('test_result/crop_test'+'.csv', np_crop, fmt = "%f,%f,%f")
np.savetxt('test_result/fake_test'+'.csv', np_fake, fmt = "%f,%f,%f")
np.savetxt('test_result/crop_test'+'.txt', np_crop, fmt = "%f,%f,%f")
np.savetxt('test_result/fake_test'+'.txt', np_fake, fmt = "%f,%f,%f")
